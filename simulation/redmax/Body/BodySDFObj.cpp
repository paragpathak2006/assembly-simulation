...
BodySDFObj::BodySDFObj(
    ...
    if (load_sdf) {
        //std::cout << "Loading SDF Object from: " << _filename.substr(0, _filename.size()-4) + std::string("_object.sdf") << std::endl;
        sdf_loaded = load_SDF();
        if (!sdf_loaded) {
            //std::cout << "Could not load SDF Object" << std::endl;
        }
    }
    if (!sdf_loaded) {
        //std::cout << "Precomputing SDF Object" << std::endl;
        precompute_SDF(dx, res);
        if (save_sdf) {
            //std::cout << "Saving SDF Object to: " << _filename.substr(0, _filename.size()-4) + std::string("_object.sdf") << std::endl;
            save_SDF();
        }
    }
...
bool BodySDFObj::load_SDF() {
    std::string inname = _filename.substr(0, _filename.size()-4) + std::string("_object.sdf");
...
void BodySDFObj::save_SDF() {
    std::string outname = _filename.substr(0, _filename.size()-4) + std::string("_object.sdf");    
...
void BodySDFObj::clear_saved_SDF() {
    std::string name = _filename.substr(0, _filename.size()-4) + std::string("_object.sdf");    
    remove(name.c_str());
}
