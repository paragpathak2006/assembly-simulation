...
SDFMesh::SDFMesh(Matrix3X V, Matrix3Xi F, dtype dx, std::string load_path, std::string save_path): Mesh(V, F) {
    bool loaded = false;
    if (load_path != "") {
        //std::cout << "Loading SDF Mesh from: " << load_path << std::endl;
        loaded = load_SDF(load_path);
        if (!loaded) {
            //std::cout << "Could not load SDF Mesh" << std::endl;
        }
    }
    if (!loaded) {
        //std::cout << "Precomputing SDF Mesh" << std::endl;
        precompute_SDF(dx);
        if (save_path != "") {
            //std::cout << "Saving SDF Mesh to: " << save_path << std::endl;
            save_SDF(save_path);
        }
    }
}
