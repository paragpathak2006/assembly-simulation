...
int Simulation::replay_next(Simulation prev_sim, bool first) {
    if (_q_his.size() == 0) {
        std::cerr << "[Error] Please call simulation.reset() before simulation.run_viewer()." << std::endl;
        throw "error";
    }
    ...
    //     for (int i = 0; i < prev_sim.object_ids.size(); i++) {
    //         viewer.RemoveObject(prev_sim.object_ids[i]);
    //     }
    ...
    //     std::vector<Matrix3Xi> face_list;
    //     std::vector<opengl_viewer::Option> option_list;
    //     _viewer->_sim->get_rendering_objects(vertex_list, face_list, option_list, _viewer->_animator_list);
    //     for (int i = 0;i < vertex_list.size();i++) {
    //         object_ids.push_back(viewer.AddDynamicObject(vertex_list[i], face_list[i], _viewer->_animator_list[i], 
    //             option_list[i]));
    //     }
    //     //
