...
Joint* Simulation::parse_from_xml_file(pugi::xml_node root, pugi::xml_node node,
    ...
    if (verbose) {
        std::cerr << "enter: " << node.name() << " top" << std::endl;
    }
    ...
    if ((std::string)(node.name()) == "redmax") {
        ...
        if (node.child("joint")) {
            if (verbose) {
                std::cerr << "child is joint" << std::endl;
            }
        ...
        if (node.child("body")) {
            if (verbose) {
                std::cerr << "child is body" << std::endl;
            }
        ...
        for (auto child : node.children())
            if ((std::string)(child.name()) == "link") {
                if (verbose) {
                    std::cerr << "child is link 1" << std::endl;
                }
                parse_from_xml_file(root, child, joint, joint_cnt, verbose);
                if (verbose) {
                    std::cerr << "link returned 1" << std::endl;
                }
            }
        ...
        for (auto child : node.children())
            if ((std::string)(child.name()) == "link") {
                if (verbose) {
                    std::cerr << "child is link 2" << std::endl;
                }
                root_joint = parse_from_xml_file(root, child, parent_joint, joint_cnt, verbose);
                if (verbose) {
                    std::cerr << "link returned 2" << std::endl;
                }
            }
        return root_joint;
    }
